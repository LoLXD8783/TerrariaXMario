...
internal class FireFlowerFireball : MetaballProjectile, IDrawToDustMetaballsTarget
{
    ...
    public override bool OnTileCollide(Vector2 oldVelocity)
    {
        ...
        {
            Projectile.velocity.Y = bounceSpeed * Projectile.scale;
            tileCollideCount++;
            ...
            if (PairedMetaballDust != null)
            {
                for (int i = 0; i < 5; i++)
                {
                    ...
                    dust.scale = Main.rand.NextFloat(0.7f, 0.8f) * Projectile.scale;
                }
            }
    ...
    {
        if (source.Context == "Big Fireball") Projectile.scale *= 2;
    }
